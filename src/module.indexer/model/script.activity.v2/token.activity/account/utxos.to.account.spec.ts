...
describe('utxos-to-account', () => {
  it('should be indexed to both sender and receiver as two unique activityV2', async () => {
    ...
    expect(senderActivities.length).toStrictEqual(1)
    const newActivity = senderActivities[0]
    expect(newActivity.dftx?.type).toStrictEqual('utxos-to-account-gain')
  ...
  it('multiple output address address should be indexed as under different hid', async () => {
    ...
    expect(dest1Activity?.id).toContain(`${encodedHeight}${txid}ff`)
