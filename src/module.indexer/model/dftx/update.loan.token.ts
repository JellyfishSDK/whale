...
export class UpdateLoanTokenIndexer extends DfTxIndexer<UpdateLoanToken> {
  ...
  async indexTransaction (block: RawBlock, txn: DfTxTransaction<UpdateLoanToken>): Promise<void> {
    const { data } = txn.dftx
  ...
  async invalidateTransaction (block: RawBlock, txn: DfTxTransaction<UpdateLoanToken>): Promise<void> {
    const { data } = txn.dftx
    const [existingLoanToken, existingToken] = await this._findExisting(data.tokenTx)
