...
import { UTXO } from '@defichain/jellyfish-api-core/dist/category/oracle'
...
describe('oracle', () => {
  ...
  it('should list with UTXOs', async () => {
    ...
    const oracleid1 = await controller.appointOracle(address, priceFeeds, { weightage: 1, utxos: inputs })
    const oracleid2 = await controller.appointOracle(address, priceFeeds, { weightage: 2, utxos: inputs })
    const oracleid3 = await controller.appointOracle(address, priceFeeds, { weightage: 2, utxos: inputs })
    const oracleid4 = await controller.appointOracle(address, priceFeeds, { weightage: 3, utxos: inputs })

    await container.generate(1)

    // removeoracle
    await controller.removeOracle(oracleid4, inputs)

    await container.generate(1)

    // updateoracle
    await controller.updateOracle(oracleid3, address, {
      priceFeeds: [{ token: 'TESLA', currency: 'USD' }],
      weightage: 3,
      utxos: inputs
    })
    ...
    const prices1 = [{ tokenAmount: '0.5@APPLE', currency: 'EUR' }]
    await client.oracle.setOracleData(oracleid1, timestamp, { prices: prices1, utxos: inputs })

    const prices2 = [{ tokenAmount: '1.0@APPLE', currency: 'EUR' }]
    await client.oracle.setOracleData(oracleid2, timestamp, { prices: prices2, utxos: inputs })

    const prices3 = [{ tokenAmount: '1.5@TESLA', currency: 'USD' }]
    await client.oracle.setOracleData(oracleid3, timestamp, { prices: prices3, utxos: inputs })
